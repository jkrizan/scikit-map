...
class CfCCell(nn.Module):
    def __init__(
        ...
        if len(backbone_layers) > 0:
            layer_list = [
                nn.Linear(input_size + hidden_size, backbone_layers[0]),
                backbone_activation(),
            ]
            if backbone_dropout > 0.0:
                    layer_list.append(torch.nn.Dropout(backbone_dropout))
            for i in range(1, len(backbone_layers)):
                layer_list.append(nn.Linear(backbone_layers[i - 1], backbone_layers[i]))
        ...
        cat_shape = int(
            self.hidden_size + input_size if len(backbone_layers) == 0 else backbone_layers[-1]
        )
        ...
        if self.mode == "pure":
            ...
            self.time_a = nn.Linear(cat_shape, hidden_size) # cat_shape, batch_size
            self.time_b = nn.Linear(cat_shape, hidden_size) # cat_shape, batch_size
        self.init_weights()
