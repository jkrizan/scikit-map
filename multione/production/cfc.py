...
from settings import N_IMAGES_PER_YEAR
...
class MemoryDataLoader:
    ...
    def __next__(self):
        if self.queue is not None:
            batch = self.queue.get(block=True, timeout=None)  # type: ignore
        else:
    ...
    def shuffler(self):
        indices = (
            self.indexes.cpu().numpy()
            if isinstance(self.indexes, torch.Tensor)
            else self.indexes
        )
        self.rng.shuffle(indices)
        self.indexes = (
            torch.tensor(indices, device=self.indexes.device)
            if isinstance(self.indexes, torch.Tensor)
            else indices
        )
...
class ArcoV2Dataset(Dataset):
    ...
    def __init__(
        ...
        for y in self.years:
            dates = [
                datetime(y, 1, 8) + timedelta(days=16 * i)
                for i in range(N_IMAGES_PER_YEAR)
            ]
            dates_list.extend(dates)
        self.days_from_start = torch.tensor(
            np.array([(d - datetime(self.years[0], 1, 1)).days + 1 for d in dates_list])
        )
        self.dates = np.array(dates_list).astype("datetime64[D]")
        self.ind_months = np.array([d.month for d in self.dates.astype(object)])
        self.ind_doys_cpu = np.arange(len(self.dates)) % N_IMAGES_PER_YEAR
        ...
        if self.zarr_path is not None:
            ...
            self.tiles = generator.permutation(
                tiles
            )  # to get always same order of tiles
            ...
            with ThreadPoolExecutor(max_workers=self.nthreads) as executor:
                futures = [
                    executor.submit(self._read_tile_from_zarr, tile, tj)
                    for tj, tile in enumerate(self.tiles)
                ]
                for future in tqdm(
                    as_completed(futures), total=len(futures), desc="Reading tiles"
                ):
                    ...
                    if len(tile_nts) == 0:
                        ...
                        for pj in range(tile_nts.shape[0]):
                            self.pixel_indices_list.extend(
                                [(tj, pj, jts) for jts in range(tile_nts[pj] - 1)]
                            )
                            self.ncases += tile_nts[pj] - 1

                        data: List = [
                            torch.tensor(d, dtype=self.dtype) for d in tile_data[1:-1]
                        ]
                        if tile_data[-1] is not None:
                            data.append(
                                tile_data[-1].astype(bool)
                            )  # all_valid_values as boolean
                        else:
                            data.append(None)
            ...
            self.pixel_indices: np.ndarray = np.array(
                self.pixel_indices_list, dtype=np.int32
            )  # (tile_ind, pixel_ind, ts_ind)
    ...
    def _read_tile_from_zarr(self, tile, tj: int):
        ...
        indices_data = np.empty(
            (self.n_output, lsdata.shape[1], lsdata.shape[2]), dtype=np.float32
        )
        ...
        if self.prepared_all_cases:
            if self.return_tensors:
                return (
                    self.all_y[idx],
                    self.all_x[idx],
                    self.all_x_gtemp[idx],
                    self.all_timespans[idx],
                )
            else:
                return (
                    self.all_y[idx].cpu().numpy(),
                    self.all_x[idx].cpu().numpy(),
                    self.all_x_gtemp[idx].cpu().numpy(),
                    self.all_timespans[idx].cpu().numpy(),
                )
    ...
    def prepare_all_cases(self):
            ...
            for pix_ind in prange(npix):
                ...
                for ts_ind in range(1, tile_nts[pix_ind]):
                    ...
                    for band_ind in range(nbands):
                        x[ncase, :, band_ind] = j_lsdata[
                            band_ind, ts_ind : ts_ind + sequence_length
                        ]
                    ...
                    timespans_data[ncase, :] = (
                        j_dates[ts_ind : ts_ind + 1 + sequence_length]
                        - j_dates[ts_ind - 1 : ts_ind + sequence_length]
                    ) / 366
        ...
        for t in tqdm(range(len(self.tiles)), desc="Preparing tiles"):
            ...
            timespans_data = torch.tensor(timespans_data, dtype=self.dtype).to(
                self.device
            )
    ...
    def get_one_pixel_timeseries(self, tile_ind: int, pix_ind: int):
        ...
        (lsdata, msdata, gtemp, gtemp_min, gtemp_max, all_valid_values, tile_nts) = (
            self.data[tile_ind]
        )
        ...
        x = (
            torch.stack(xx, dim=0).permute(0, 2, 1).to(self.dtype)
        )  # (n_dates, seq_len, n_features)
        y = lsdata[:, :, pix_ind].T
    ...
    def prepare_for_ray_worker(
        self,
        rank: int,
        nranks: int,
        ncases_validation: float,
        device: str | torch.device,
    ) -> tuple[Self, np.ndarray, np.ndarray]:
        train_indices, val_indices = self.get_train_validation_indices(
            ncases_validation=ncases_validation
        )
...
class CfcModel(nn.Module):
    def __init__(
        ...
        self.lstm = LSTMCell(
            self.input_size + self.timeless_input_size, self.hidden_size
        )  # Mixed memory
    ...
    def inference(self, x, timeless, timespans):
        # device = x.device
        dtype = x.dtype
