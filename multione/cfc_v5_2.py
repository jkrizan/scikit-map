...
class ArcoV2DatasetV3(Dataset):
    ...
    def get_one_case(self, idx: int):
        ...
        res = [y.astype(np.float16), x.astype(np.float16), timespans.astype(np.float16)]
        if self.read_timeless:
            x_timeless = covariates[:,pix_ind]  # type: ignore
            res.append(x_timeless.astype(np.float16))
...
class CfcLearner_v5(pl.LightningModule):
    ...
    def setup(self, stage=None):
        ...
        if self.hparams['debug']:
            ...
            self.train_loader = DataLoader(train_subset, batch_size=8192, shuffle=True, num_workers=8)
            self.val_loader = DataLoader(valid_subset, batch_size=8192, shuffle=False, num_workers=8)
