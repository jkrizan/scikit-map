...
class Objective:
    ...
    def __init__(self, gpu_queue: GpuQueue, fld_save_models: Path):
        ...
        #DEVICES = [0,1,2,3]
        self.INDICES = ['ndvi']; 
        self.OUTPUT_SIZE = len(self.INDICES)
        self.INPUT_SIZE = len(self.INDICES) + 2
        ...
        self.dataset = ArcoV2DatasetV9( self.FN_ZARR,
                        years=self.YEARS,
    ...
    def get_train_val_loaders(self, gpu_i: int) -> tuple[MemoryDataLoader, MemoryDataLoader]:
        ...
        train_loader = MemoryDataLoader(self.datasets[gpu_i], batch_size=self.BATCHSIZE, indexes=train_inds, cache_length=1000)
        val_loader = MemoryDataLoader(self.datasets[gpu_i], batch_size=self.BATCHSIZE, indexes=valid_inds, cache_length=1000)
    ...
    def __call__(self, trial: Trial):
        with self.gpu_queue.one_gpu_per_process() as gpu_i:
            ...
            max_backbone_layer_size = 72 #(2 ** n_backbone_layers ) * 6 # max = 256
            ...
            model = CfcModelV9(input_size=self.INPUT_SIZE,
...
def train_v9_hptuning():
    study_name = "cfc_v9_opt_1"
    fld_save_models = Path(__file__).parent / "optuna_models_v9"/ study_name
    fld_save_models.mkdir(parents=True, exist_ok=True)

    study = optuna.create_study(storage="sqlite:///cfc_v9_opt.sqlite3", direction="minimize", study_name=study_name, load_if_exists=True)
    optuna.logging.get_logger("optuna").addHandler(logging.StreamHandler(sys.stdout))
    study.optimize(Objective(GpuQueue(), fld_save_models), n_trials=1000, timeout=None, n_jobs=64)   
    ### !!!!!!!! ####
