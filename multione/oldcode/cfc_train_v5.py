...
def train_test_v5_5():
    ...
    checkpoint_callback = ModelCheckpoint(
        # dirpath=checkpoints_path, # <--- specify this on the trainer itself for version control
        filename="cfc_v5.5_e{epoch:03d}_{val_loss:.2f}",
        every_n_epochs=1,
        save_top_k=-1,  # <--- this is important!
    )
    trainer = pl.Trainer(max_epochs=300,
...
def train_test_v5_2_continue():
    ...
    trainer.fit(learner, ckpt_path="/home/josip/scikit-map/multione/lightning_logs/version_28/checkpoints/cfc_v5.2_eepoch=099.ckpt") 
